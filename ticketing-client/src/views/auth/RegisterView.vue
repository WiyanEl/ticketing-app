    ...
    function register() {
      ...
      axios
        ...
        .catch((err) => {
          ...
          btnRegister.removeChild(btnRegister.children[0]);
          btnRegister.innerHTML = `
          <span>Resgister <i class="bi bi-box-arrow-in-right"></i></span>
        `;
        });
    }
      ...
      <div class="row justify-content-center">
        <div class="col-12 col-md-4">
          <form @submit.prevent="register">
            <h1 class="h3 mb-4 fw-normal text-center text-uppercase">Register</h1>

            <div class="mb-3">
              <input v-model="user.name" type="text" class="form-control" placeholder="Nama" />
            ...
            <div class="mb-3">
              <input v-model="user.password_confirmation" type="password" class="form-control" placeholder="Konfirmasi Password" />
              <span v-if="validation.password_confirmation" class="text-danger">
                {{ validation.password_confirmation[0] }}
              </span>
            </div>
            <div class="mb-3">
              <button class="w-100 btn btn-lg btn-success btn-register" type="submit">
                <span>Resgister <i class="bi bi-person-plus"></i></span>
              </button>
            </div>
            <div class="mb-3 text-center">
              <span>Sudah punya akun? <router-link :to="{ name: 'admin.login' }" class="text-success text-decoration-none">Login</router-link></span>
            </div>
          </form>
        </div>
      </div>
